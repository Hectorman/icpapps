parasails.registerPage('bienvenido', {
  ...
  beforeMount: function() {
    ...
    if ( this.me.rolComisiones == 'Gerente' || this.me.rolComisiones == 'Jefe Dpto' ) {

      this.listaGerencias = [{ valor: this.me.gerencia }];

    }

    for (let index = 0; index < this.comisionesAprobadas.length; index++) {
      const comision = this.comisionesAprobadas[index];

      this.listaDestinos.push( comision.ciudad );
  ...
  mounted: async function() {
    ...
    $('.datepicker-input').datepicker({
      ...
      onSelect: function(formattedDate, date, inst) {
        ...
        self.filtrosTendencia[ inst.$el[0].dataset.name ] = formattedDate;
  ...
  afterNavigate: async function(virtualPageSlug){
    ...
    } else if ( virtualPageSlug == 'tendencias' ) {

      var self = this;

      var $activeVPage = $('#tendencias-page');
    
      setTimeout(function(){ 

        $activeVPage.show(function(){
  ...
  methods: {
    ...
    updateDates: function(){
      ...
      if ( this.filtrosTendencia.fechaInicial ) {

        var initTimestamp = Date.parse( this.filtrosTendencia.fechaInicial.replace(new RegExp('/', 'g'), ',') );
      ...
      if ( this.filtrosTendencia.fechaFinal ) {

        var finalTimestamp = Date.parse( this.filtrosTendencia.fechaFinal.replace(new RegExp('/', 'g'), ',') );
    ...
    makeTendencia: function() {
      ...
      console.log( this.comisionesAprobadas );

      console.log( this.filtrosTendencia.gerencia );
      ...
      for (let index = 0; index < this.comisionesAprobadas.length; index++) {
        ...
        if ( this.filtrosTendencia.gerencia != 'todos' && this.filtrosTendencia.gerencia != comision.gerencia.toLowerCase() ) greenLight = false;

        if ( this.filtrosTendencia.destino != 'todos' && this.filtrosTendencia.destino != comision.ciudad ) greenLight = false;

        if ( this.filtrosTendencia.fechaInicial ) {

          var fechaInicialList = this.filtrosTendencia.fechaInicial.split('/');

          if ( fechaInicialList[1] > 1 ) fechaInicialList[1]--;

          var fechaInicialStamp = new Date(Date.UTC(fechaInicialList[0],fechaInicialList[1],'1'));
          
          fechaInicialStamp = fechaInicialStamp.getTime();

          if ( fechaComision < fechaInicialStamp ) greenLight = false;

        }

        if ( this.filtrosTendencia.fechaFinal ) {

          var fechaFinalList = this.filtrosTendencia.fechaFinal.split('/');

          var fechaFinalStamp = new Date(Date.UTC(fechaFinalList[0],fechaFinalList[1],'1'));
          
          fechaFinalStamp = fechaFinalStamp.getTime();

          if ( fechaComision > fechaFinalStamp ) greenLight = false;
    ...
    openRechazoPopup: function(e) {
      ...
      this.comisionToUpdate = {
        action: action,
        idComision: idComision * 1,
        motivoRechazo: $('.rechazo-text').val()
      }
    ...
    updateComision: async function(e) {
      ...
      console.log( this.comisionToUpdate );

      var result = await Cloud['updateComision'].with( this.comisionToUpdate ).tolerate((err)=>{
      ...
      if ( result ) {
        ...
        Swal.fire(
          ...
          'Comisión actualizada exitósamente.',
          'success'
        )
